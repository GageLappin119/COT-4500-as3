...
def Runge_Kutta(range_start, range_end, iterations, initial_point_y, f, h):
    ...
    for i in range(iterations):
        k_1 = h * f(range_start + (h * i), values[i])
        k_2 = h * f(range_start + (h * i) + h/2, values[i] + k_1/2)
        k_3 = h * f(range_start + (h * i) + h/2, values[i] + k_2/2)
        k_4 = h * f(range_start + (h * i) + h, values[i] + k_3)
